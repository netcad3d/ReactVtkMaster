...
var VectorMode = vtkScalarsToColors$1.VectorMode;
...
function defaultAutoLayout(publicAPI, model) {
  return function(helper) {
    ...
    var textSizes = helper.updateTextureAtlas();
    helper.setTopTitle(false);
    var boxSize = helper.getBoxSizeByReference();
    if (helper.getLastAspectRatio() > 1) {
      ...
      var titleHeight = 2 * (0.8 * tickTextStyle.fontSize + textSizes.titleHeight + helper.getAxisTitlePixelOffset()) / lastSize[1];
      var tickWidth = 2 * textSizes.tickWidth / lastSize[0];
...
function defaultGenerateTicks(publicApi, model) {
  return function(helper) {
    ...
    var scale = linear2().domain([lastTickBounds[0], lastTickBounds[1]]);
    var ticks2 = scale.ticks(5);
    var format2 = scale.tickFormat(5);
...
function vtkScalarBarActorHelper(publicAPI, model) {
  ...
  publicAPI.updateAPISpecificData = function(size, camera, renderWindow) {
    ...
    if (model.forceUpdate || Math.max(scalarsToColors.getMTime(), publicAPI.getMTime(), model.renderable.getMTime()) > model.lastRebuildTime.getMTime()) {
      var range2 = scalarsToColors.getMappingRange();
      model.lastTickBounds = _toConsumableArray(range2);
      ...
      if (model.renderable.getAutomated()) {
        ...
        var textSizes = publicAPI.updateTextureAtlas();
  ...
  publicAPI.updateTextureAtlas = function() {
    ...
    var results = {};
    var newTmAtlas = /* @__PURE__ */ new Map();
    var maxWidth = 0;
    var totalHeight = 1;
  ...
  publicAPI.computeBarSize = function(textSizes) {
    ...
    var tickHeight = 2 * textSizes.tickHeight / model.lastSize[1];
    var segSize = [1, 1];
    if (model.vertical) {
      var tickWidth = 2 * (textSizes.tickWidth + model.tickLabelPixelOffset) / model.lastSize[0];
      if (model.topTitle) {
        var titleHeight = 2 * (textSizes.titleHeight + model.axisTitlePixelOffset) / model.lastSize[1];
        model.barSize[0] = model.boxSize[0] - tickWidth;
        model.barSize[1] = model.boxSize[1] - titleHeight;
      } else {
        var titleWidth = 2 * (textSizes.titleHeight + model.axisTitlePixelOffset) / model.lastSize[0];
      ...
      var _tickWidth = (2 * textSizes.tickWidth - 8) / model.lastSize[0];
      var _titleHeight = 2 * (textSizes.titleHeight + model.axisTitlePixelOffset) / model.lastSize[1];
  ...
  publicAPI.recomputeBarSegments = function(textSizes) {
    ...
    var haveAbove = (_model$renderable$get3 = (_model$renderable$get4 = model.renderable.getScalarsToColors()).getUseAboveRangeColor) === null || _model$renderable$get3 === void 0 ? void 0 : _model$renderable$get3.call(_model$renderable$get4);
  ...
  publicAPI.createPolyDataForOneLabel = function(text, pos, alignment, orientation, offset, results) {
    ...
    if (orientation === "vertical") {
      ...
      if (alignment[0] === "middle") {
        tmp2v3[1] -= value.width / 2;
      } else if (alignment[0] === "right") {
        tmp2v3[1] -= value.width;
      }
      if (alignment[1] === "middle") {
        tmp2v3[0] += value.height / 2;
      ...
      textSize[1] = value.height;
      if (alignment[0] === "middle") {
        tmp2v3[0] -= value.width / 2;
      } else if (alignment[0] === "right") {
        tmp2v3[0] -= value.width;
      }
      if (alignment[1] === "middle") {
        tmp2v3[1] -= value.height / 2;
      } else if (alignment[1] === "top") {
  ...
  publicAPI.updatePolyDataForLabels = function() {
    ...
    var ticks2 = publicAPI.getTicks();
    var tickStrings = publicAPI.getTickStrings();
    for (var t = 0; t < ticks2.length; t++) {
      var tickPos = (ticks2[t] - model.lastTickBounds[0]) / (model.lastTickBounds[1] - model.lastTickBounds[0]);
...
function extend2(publicAPI, model) {
  ...
  if (!model.generateTicks)
    model.generateTicks = defaultGenerateTicks();
...
var newInstance = macro.newInstance(extend2, "vtkScalarBarActor");
...
//# sourceMappingURL=chunk-FNZYZ4NE.js.map
